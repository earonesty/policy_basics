...
class TimeArgs:
    ...
    def __init__(self, days: List[int], include: List[date], exclude: List[date], time_start: Optional[time] = None,
    ...
    def strs_to_dates(strs: List[str]):
        assert type(strs) is list, "input must be a list"
    ...
    def strs_to_time(tim: Optional[str]):
        if not tim:
            return None
        t = dateutil.parser.parse(tim).timetz()
        if not t.tzinfo:
            t = t.replace(tzinfo=LOCAL_TIMEZONE)
        return t
